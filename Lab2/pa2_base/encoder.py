...
class Encoder:
    ...
    def parseFiles(self):
        ...
        with open(self.policyFile) as pf:
            self.opponent = pf.readline().rstrip()
            self.self = '1' if self.opponent=='2' else '2'
    ...
    def formulateMDP(self):
        ...
        for state, idx in self.states.items():
            for ac in range(numActions):
                if state[ac] != '0':
                    MDP_TRANSITIONS[idx][ac].append((end, -1.0, 1.0))
                    continue
                nextstate = state[:ac] + self.self + state[(ac+1):]

                if isTerminal(nextstate, pl=self.self) or ('0' not in nextstate):
                ...
                for ac2, ac2_prob in enumerate(self.policies[nextstate]):
                    if ac2_prob == 0:
                        continue
                    ns2 = nextstate[:ac2] + self.opponent + nextstate[(ac2+1):]
                    
                    if isTerminal(ns2, pl=self.opponent):
